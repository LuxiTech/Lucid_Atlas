...
namespace
{
std::string currentSystemTimeForFilename()
{
...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr &node)
{
  ...
  try_declare.template operator()<bool>("time_offset.auto_img_time_offset", false);
  ...
  try_declare.template operator()<int>("publish.image_skip_num", 1);
  ...
  this->node->get_parameter("time_offset.auto_img_time_offset", auto_img_time_offset);
  img_time_offset_calibrated = !auto_img_time_offset;
  ...
  this->node->get_parameter("publish.image_skip_num", image_skip_num);
  ...
  if (image_skip_num < 1) image_skip_num = 1;
...
void LIVMapper::initializeSubscribersAndPublishers(rclcpp::Node::SharedPtr &node, image_transport::ImageTransport &it_)
{
  ...
  auto lidar_qos = rclcpp::QoS(rclcpp::KeepLast(3)).reliable().durability_volatile();
  auto imu_qos = rclcpp::QoS(rclcpp::KeepLast(200)).reliable().durability_volatile();
  auto image_qos = rclcpp::QoS(rclcpp::KeepLast(3)).reliable().durability_volatile();
  ...
  auto pointcloud_qos = rclcpp::QoS(1).reliable();
  auto marker_qos = rclcpp::QoS(1).best_effort();
  pubLaserCloudFullRes = this->node->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_registered", pointcloud_qos);
  pubNormal = this->node->create_publisher<visualization_msgs::msg::MarkerArray>("/visualization_marker", marker_qos);
  pubSubVisualMap = this->node->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_visual_sub_map_before", pointcloud_qos);
  pubLaserCloudEffect = this->node->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_effected", pointcloud_qos);
  pubLaserCloudMap = this->node->create_publisher<sensor_msgs::msg::PointCloud2>("/Laser_map", pointcloud_qos);
  ...
  pubLaserCloudDyn = this->node->create_publisher<sensor_msgs::msg::PointCloud2>("/dyn_obj", pointcloud_qos);
  pubLaserCloudDynRmed = this->node->create_publisher<sensor_msgs::msg::PointCloud2>("/dyn_obj_removed", pointcloud_qos);
  pubLaserCloudDynDbg = this->node->create_publisher<sensor_msgs::msg::PointCloud2>("/dyn_obj_dbg_hist", pointcloud_qos);
  mavros_pose_publisher = this->node->create_publisher<geometry_msgs::msg::PoseStamped>("/mavros/vision_pose/pose", 10);
  pubImage = it.advertise("/rgb_img", 1);
  pubImuPropOdom = this->node->create_publisher<nav_msgs::msg::Odometry>("/LIVO2/imu_propagate", 100);
  imu_prop_timer = this->node->create_wall_timer(0.004s, std::bind(&LIVMapper::imu_prop_callback, this));
  voxelmap_manager->voxel_map_pub_= this->node->create_publisher<visualization_msgs::msg::MarkerArray>("/planes", marker_qos);
}
...
void LIVMapper::handleVIO() 
{
  ...
  {
    // No visual points in the first few frames is normal.
    return;
  }
    
  // Quiet during live mapping.
...
{    
  ...
  {
    // No LiDAR points in the first few frames is normal.
    return;
  }
...
void LIVMapper::savePCD() 
{
  ...
  {
    const std::string time_tag = currentSystemTimeForFilename();
    std::string raw_points_dir = std::string(ROOT_DIR) + "Log/PCD/all_raw_points_" + time_tag + ".pcd";
    std::string downsampled_points_dir = std::string(ROOT_DIR) + "Log/PCD/all_downsampled_points_" + time_tag + ".pcd";
...
void LIVMapper::livox_pcl_cbk(const livox_ros_driver2::msg::CustomMsg::ConstSharedPtr &msg_in)
{
  ...
  RCLCPP_DEBUG(this->node->get_logger(), "\033[95mSelf sync IMU and LiDAR, HARD time lag is %.10lf \n\033[0m", timediff_imu_wrt_lidar - 0.100);
...
void LIVMapper::imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr &msg_in)
{
  ...
  if (!ros_driver_fix_en && timestamp < last_timestamp_lidar - 0.3)
  {
    RCLCPP_WARN_THROTTLE(
        this->node->get_logger(),
        *this->node->get_clock(),
        2000,
        "Drop stale IMU. delta time: %lf .",
  ...
  {
    const double imu_loopback = last_timestamp_imu - timestamp;
    const double lidar_imu_delta = fabs(last_timestamp_lidar - timestamp);
    if (lidar_imu_delta < 0.5)
    {
      RCLCPP_WARN_THROTTLE(
          this->node->get_logger(),
          *this->node->get_clock(),
          2000,
          "imu time stamp loops back %.4lf seconds, but it is close to current LiDAR time "
          "(delta %.4lf). Reset IMU buffer and continue.",
          imu_loopback, lidar_imu_delta);
      imu_buffer.clear();
    }
    else
    {
      mtx_buffer.unlock();
      sig_buffer.notify_all();
      RCLCPP_WARN_THROTTLE(this->node->get_logger(), *this->node->get_clock(), 2000, "imu loop back, offset: %lf \n", imu_loopback);
      return;
    }
  }
  ...
  {
    const double imu_jump = timestamp - last_timestamp_imu;
    const double lidar_imu_delta = fabs(last_timestamp_lidar - timestamp);
    if (lidar_imu_delta < 0.5)
    {
      RCLCPP_WARN_THROTTLE(
          ...
          *this->node->get_clock(),
          2000,
          "imu time stamp jumps %.4lf seconds, but it is close to current LiDAR time "
          "(delta %.4lf). Reset IMU buffer and continue.",
          imu_jump, lidar_imu_delta);
      imu_buffer.clear();
    }
    else
    {
      RCLCPP_WARN_THROTTLE(this->node->get_logger(), *this->node->get_clock(), 2000, "imu time stamp Jumps %0.4lf seconds \n", imu_jump);
      mtx_buffer.unlock();
      sig_buffer.notify_all();
      return;
    }
...
bool LIVMapper::calibrateImageTimeOffsetIfNeeded(double raw_img_time)
{
  if (!auto_img_time_offset || img_time_offset_calibrated) return true;
  ...
  if (last_timestamp_lidar < 0.0)
  {
    RCLCPP_INFO_THROTTLE(
        ...
        *this->node->get_clock(),
        2000,
        "Waiting for LiDAR before auto image time offset calibration.");
    return false;
  }

  img_time_offset = last_timestamp_lidar - raw_img_time;
  img_time_offset_calibrated = true;
  RCLCPP_WARN(
      this->node->get_logger(),
      "Auto image time offset calibrated: %.9f = lidar %.9f - image %.9f",
      img_time_offset,
...
void LIVMapper::img_cbk(const sensor_msgs::msg::Image::ConstSharedPtr &msg_in)
{
  ...
  const double raw_img_time = stamp2Sec(msg->header.stamp);
  if (!calibrateImageTimeOffsetIfNeeded(raw_img_time)) return;

  double msg_header_time = raw_img_time + img_time_offset;
  ...
  if (image_skip_num > 1 && (image_skip_count++ % image_skip_num) != 0)
  {
    return;
  }
...
void LIVMapper::publish_frame_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
  ...
  if(flush_wait_pub) PointCloudXYZI().swap(*pcl_wait_pub);
  PointCloudXYZI().swap(*pcl_w_wait_pub);
}
