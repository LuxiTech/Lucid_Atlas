...
## 4.1 Luxi Device Preview

This repository also provides a Luxi bringup launch for checking the real device topics before starting FAST-LIVO2 mapping:
...
It starts the device workspaces under `/home/nvidia/project/luxi-atlas/device`:

- Hikrobot `MV-CU013-A0UC` camera node
- Livox `MID360` driver node
- RViz2 preview

Expected preview topics:

```text
/hikrobot/cu013/rgb_img  sensor_msgs/msg/Image
/livox/lidar             sensor_msgs/msg/PointCloud2 by default
/livox/imu               sensor_msgs/msg/Imu
```

For FAST-LIVO2 CustomMsg compatibility, launch with:

```bash
ros2 launch fast_livo luxi_device_cu013_mid360_preview.launch.py mid360_xfer_format:=1
```

`mid360_xfer_format:=0` is better for RViz point cloud preview. `mid360_xfer_format:=1` is better for feeding FAST-LIVO2.
