...
public:
  ...
  : Node("web_api_node")
  {
    ...
    declare_parameter<double>("converted_map_min_z", 0.15);
    declare_parameter<double>("converted_map_max_z", 1.60);
    declare_parameter<int>("converted_map_min_component_cells", 20);
    declare_parameter<std::string>("converted_map_suffix", "_nav_floor_plane_autofit_clean20");
...
private:
    ...
    const http::request<http::string_body> & req)
  {
    ...
    if (req.method() == http::verb::post && target == "/api/map/initial_pose") {
      ...
      return make_json_response(req, http::status::ok, "{\"ok\":true,\"topic\":\"/initialpose\"}");
    }
