...
def generate_launch_description():
    ...
    declare_state_estimation = DeclareLaunchArgument(
        "state_estimation",
        default_value="false",
        description="Run CHAMP state estimation and EKF odometry",
    )
    declare_ground_truth_odom = DeclareLaunchArgument(
        "ground_truth_odom",
        default_value="true",
        description="Bridge Gazebo ground truth to /odom and TF",
    )
    ...
    gazebo_ld = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            "headless": PythonExpression(
                ["'", LaunchConfiguration("gui"), "'.lower() == 'false'"]
            ),
            "close_loop_odom": "true",
        }.items(),
    )
    ...
    ground_truth_odom_bridge = Node(
        ...
        condition=IfCondition(LaunchConfiguration("ground_truth_odom")),
    )
